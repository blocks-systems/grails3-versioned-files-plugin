...
class AnnexableTagLib {
    ...
    def annexesDomainPanel = { attrs, body ->
        ...
        if (bean?.metaClass?.hasProperty(bean, 'annexes') != null) {
            ...
            sb.append("<form id='uploadNewForm' class='MultiFile-intercepted' enctype='multipart/form-data' name='uploadNewForm' method='post' action='/annexable/addAnnex'>")
            sb.append("<input id='uploadAnnexId' type='hidden' value='' name='uploadAnnexId'>")
            sb.append("<input id='uploadBucket' type='hidden' value='' name='uploadBucket'>")
            sb.append("<label class='btn btn-default btn-file'>")
            sb.append("<span class='fa fa-upload'>")
            sb.append("<input type='file' name='uploadFile' id='uploadFile' style='display: none;'>")
            sb.append("</span>")
            sb.append("</label>")
            sb.append("</form>")
            ...
            sb.append("<span class='badge' id='annexToAttachFoundedCount' name='annexToAttachFoundedCount'>0</span>")
            ...
            bean?.getAnnexes()?.each { annex ->
                ...
                sb.append("<form id='uploadForm' class='MultiFile-intercepted' enctype='multipart/form-data' name='uploadForm' method='post' action='/annexable/uploadAnnex'>")
                sb.append("<input id='uploadAnnexId' type='hidden' value='' name='uploadAnnexId'>")
                sb.append("<input id='uploadBucket' type='hidden' value='' name='uploadBucket'>")
                sb.append("<label class='btn btn-default btn-file'>")
                sb.append("<span class='fa fa-upload'>")
            ...
            sb.append("<iframe src='/annexable/showAnnex?annexId=22' style='height:425px; width:100%;' frameborder='0' id='showAnnexiFrame' name='showAnnexiFrame'></iframe>")
            sb.append("</div>")
            sb.append("</div>")
            sb.append("</div>")
